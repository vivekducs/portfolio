  ...
  return (
    <div className="mt-4 border-t border-[var(--border-color)] pt-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h4 className="text-xs font-bold text-[var(--accent-color)] mono-font uppercase tracking-widest">
            ...
            <div key={idx} className="relative">
              ...
              {idx < arch.nodes.length - 1 && (
                <div className="hidden xl:flex absolute top-1/2 -right-2 -translate-y-1/2 z-10">
                  <ArrowRight size={12} className="text-[var(--accent-color)]/40" />
                </div>
              )}
            </div>
          );
      ...
      <div className="flex flex-wrap gap-2">
        {arch.metrics.map((m, i) => (
          <span
            key={i}
            className="text-[9px] bg-violet-500/10 border border-violet-500/20 text-[var(--accent-color)] font-bold px-2.5 py-1 rounded-full mono-font"
          >
            {m}
          </span>
        ))}
    ...
    <div>
      <button
        ...
        className={`flex items-center gap-1.5 text-[10px] font-bold mono-font transition-all cursor-pointer px-3 py-1.5 rounded-lg border ${
          open
            ? "bg-[var(--accent-color)]/15 border-[var(--accent-color)]/30 text-[var(--accent-color)]"
            : "border-[var(--border-color)] text-[var(--text-muted)] hover:border-[var(--accent-color)]/30 hover:text-[var(--accent-color)] hover:bg-[var(--accent-color)]/5"
        }`}
      >
        <Layers size={11} />
