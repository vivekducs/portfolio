...
const BOOT_LINES = [
  ...
  { text: "Loading AI Systems...", color: "text-violet-500", delay: 500 },
  { text: "Connecting to Engineering Portfolio...", color: "text-violet-500", delay: 1000 },
  { text: "Syncing GitHub repositories → vivekducs", color: "text-violet-400", delay: 1500 },
  { text: "Mapping project architectures...", color: "text-violet-400", delay: 2000 },
  { text: "Calibrating DSA performance metrics...", color: "text-violet-400", delay: 2400 },
...
export default function AIBootLoader({ onComplete }) {
  ...
  return (
    ...
    >
      ...
      {/* Central glow orbs */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-violet-500 opacity-10 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-1/3 left-1/2 -translate-x-1/2 w-64 h-64 bg-violet-500 opacity-10 rounded-full blur-[100px] pointer-events-none" />

      <div className="relative z-10 w-full max-w-lg px-8">
        {/* Logo / Name */}
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-4 relative">
            <div className="absolute inset-0 rounded-2xl bg-gradient-to-tr from-violet-600 via-violet-500 to-violet-500 opacity-80 animate-pulse" />
        ...
        {/* Terminal window */}
        <div className="bg-[#050807] border border-violet-500/20 rounded-2xl overflow-hidden shadow-2xl shadow-violet-500/5">
          {/* Terminal header bar */}
          <div className="flex items-center gap-2 px-4 py-3 border-b border-violet-500/10 bg-[#080d0b]">
            ...
            {BOOT_LINES.map((line, idx) => (
              ...
              >
                ...
                {visibleLines.includes(idx) && idx === visibleLines[visibleLines.length - 1] && idx < BOOT_LINES.length - 1 && (
                  <span className="inline-block w-2 h-3 bg-violet-500 animate-pulse rounded-sm ml-0.5" />
                )}
                {visibleLines.includes(idx) && (
                  <span className="ml-auto text-violet-400 text-[9px]">
          ...
          <div className="px-5 pb-5">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[9px] text-violet-500 mono-font font-bold">{progress}%</span>
            </div>
            <div className="w-full h-1.5 bg-violet-500/10 rounded-full overflow-hidden">
              <div
                className="h-full rounded-full bg-gradient-to-r from-violet-600 via-violet-500 to-violet-500 transition-all duration-500"
